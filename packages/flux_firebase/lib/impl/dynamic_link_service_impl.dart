...
import 'package:fstore/common/constants.dart'
    show RouteList, httpGet, isAndroid, printLog;
    ...
    try {
      ...
      if (url.contains('/product/') ||
        ...
        print(url);
         ...
         //var ss = await Services().api.getBlogByPermalink(url);
        var blog = await  getBlogByPermalink(url);
        if (blog != null) {
  ...
  Future<Blog?> getBlogByPermalink(String blogPermaLink) async {
    ...
    try {
      final response = await httpGet(
        'https://publictv.in/wp-json/wp/v2/posts?slug=$blogPermaLink'.toUri()!);
